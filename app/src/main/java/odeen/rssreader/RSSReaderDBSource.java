...
public class RSSReaderDBSource {
    ...
    public void insertNews(News news) {
        ...
        values.put(RSSReaderDBHelper.COLUMN_IMAGELINK, news.getImageLink());
        if (news.getImageLink() == null)
            Log.i("OLOLO", "OLOLO");
        mDatabase.insertOrThrow(RSSReaderDBHelper.TABLE_NAME, null, values);
    }

    public ArrayList<News> getAllNews() {
        ArrayList<News> res = new ArrayList<News>();
        Cursor cursor = mDatabase.query(RSSReaderDBHelper.TABLE_NAME, allColumns, null, null, null, null, null);
        cursor.moveToFirst();
    ...
    public void delete() {
        if (mDatabase.isOpen())
            mDatabase.delete(RSSReaderDBHelper.TABLE_NAME, null, null);
    }
}
